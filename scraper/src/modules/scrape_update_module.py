...
async def scrape_update(path: str):
    ...
    MsgPrinter.print_step_mark("Reading local data")
    ...
    if not os.path.isfile(scrape_info_path):
        MsgPrinter.print_error(
            "File 'scrape_info.json' not found",
            "ScrapeUpdate",
        )
        return
    ...
    MsgPrinter.print_tip(f"Elapsed {int(update_duration // 60)} min {round(update_duration % 60, 2)} sec.")
...
async def update_thread(tid: int, *, is_share_origin=False):
    ...
    MsgPrinter.print_step_mark("Updating forum and thread metadata", ["tid", tid])
    await asyncio.gather(
        thread_service.save_forum_info(pre_fetch_posts.forum.fid),
        thread_service.save_thread_info(pre_fetch_posts.thread),
    )

    MsgPrinter.print_step_mark("Updating posts", ["tid", tid])
    await post_service.scrape_post(pre_fetch_posts.page.total_page, is_update=True)

    MsgPrinter.print_step_mark("Completing user info", ["tid", tid])
    scrape_logger.info(generate_scrape_logger_msg("Completing user data", "StepMark", ["tid", tid]))
    await user_service.complete_user_info()

    final_treatment()
    MsgPrinter.print_step_mark("Thread update completed", ["tid", tid])
    scrape_logger.info(generate_scrape_logger_msg("Thread update completed", "StepMark", ["tid", tid]))
...
async def confirm_config(old_config: dict, new_config: dict) -> bool:
    ...
    if is_diff:
        ...
        return await questionary.confirm(
            "The current scrape config differs from the previous one. Continue?", style=WarningStyle
        ).ask_async()
    else:
        return True
